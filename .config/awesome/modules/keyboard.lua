...
local function update_status (self)
    ...
    self.widget:set_text(text:upper())
end
	...
	local icon_widget = wibox.widget {
		{
			...
			resize = false,
			image = beautiful.icon.keyboard
		},
		layout = wibox.container.margin(_, 0, 0, -1)
	}
    local text_widget = textbox()

	local composite_widget = wibarutil.compose_widget(icon_widget, text_widget, beautiful.keyboard_fg, beautiful.keyboard_bg)

    local self = widget_base.make_widget(composite_widget, nil, {enable_properties=true})
